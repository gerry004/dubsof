...
export default function PartnersSection({ variant = 'light' }: PartnersSectionProps) {
  const isDark = variant === 'dark';
  
  return (
    <div className={`overflow-hidden ${isDark ? '' : 'py-16 bg-white'}`}>
      <div className={`container mx-auto ${isDark ? 'px-0' : 'px-4 sm:px-6 lg:px-8'}`}>
        {!isDark && (
        ...
        <div className={`mx-auto relative ${isDark ? 'w-full' : 'w-3/4'}`}>
          <div className="flex overflow-hidden">
            ...
            <div className="flex animate-marquee whitespace-nowrap">
              {LOGOS.map((logo, i) => (
                <div key={i} className={`mx-4 w-24 h-12 flex items-center justify-center ${isDark ? 'filter brightness-0 invert opacity-80' : ''}`}>
                  <Image 
                    src={logo.src} 
                    alt={logo.alt} 
                    width={80} 
            ...
            <div className="flex animate-marquee whitespace-nowrap">
              {LOGOS.map((logo, i) => (
                <div key={`dup-${i}`} className={`mx-4 w-24 h-12 flex items-center justify-center ${isDark ? 'filter brightness-0 invert opacity-80' : ''}`}>
                  <Image 
                    src={logo.src} 
                    alt={logo.alt} 
                    width={80} 
